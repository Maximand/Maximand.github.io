...
We analysed two very different datasets: 27 private incident reports from an incident response company and 13 public CISA advisories based on various other incident response organizations. Two findings stood out from this data:
...
* **Across sources, the overlaps became even lower.** What CISA published and what the responders from our partnering company observed for the same group only line up part of the time. This shows a significant discrepancy between the reporting of different organizations.

These discrepancies undermine the assumption that defenders (or regulators) have access to a consistent view of attacker behaviour at the time sanctions checks are performed.
...
This rebranding breaks the link between old and new entities in legal and public intelligence contexts. While threat researchers may recognize the continuity, governments may typically require formal evidence of control or ownership to link a new group to a sanctioned predecessor. That process is slow, often opaque, and rarely public. 
...
I am not arguing for abandoning sanctions policy altogether. Instead, it would be better to propose a more realistic and risk-aware approach:
...
Sanctions policy is not wrong in its goals, but it fails in implementation. It assumes that attribution is deterministic, that indicators are authoritative, and  that defenders are equipped to make perfect decisions under pressure. None of these assumptions hold in ransomware cases.

If we want policy to support resilience rather than amplify harm, we must start by aligning it with how threat intelligence actually works. Until then, we risk not having the backs of ransomware victims, but punishing them a second time. 

This is not an argument against sanctioning ransomware actors. It is an argument for ensuring that those sanctions reach the right targets. We can't sanction what we can't see, and we shouldn't punish those who were never meant to be the target.
